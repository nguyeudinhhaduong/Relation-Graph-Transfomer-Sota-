...
def local_nodes_hetero(
    ...
    # Windows uses spawn and may re-import the training script in worker processes,
    # which can conflict with DDP initialization. Use single-process sampling there.
    if platform.system().lower() == "windows" or num_workers <= 1:
        init_worker_globals(adjacency, all_nodes_all_types)
        results = [_process_one_seed(task) for task in tasks]
    else:
